...
class Mxp_Plugins_List_Table extends WP_List_Table {
	...
	public function column_name($item) {
		...
		$dlink = isset($item['dlink']) ? $item['dlink'] : '';
		if (strpos($dlink, 'https://downloads.wordpress.org') === 0) {
	...
	public function table_data() {
		...
		for ($i = 0; $i < count($data); ++$i) {
			...
			if (!empty($wp) && $data[$i]['dlink'] == "" && property_exists($wp, 'version') && property_exists($wp, 'download_link')) {
	...
	public function column_id($item) {
		$actions = array();
		$actions['activate'] = sprintf('<button type="button" class="button button-default mxp-activate" data-id="p_%s" disabled>啟用</button>', $item['id']);
		$actions['update'] = sprintf('<button type="button" class="button button-default mxp-update" data-id="p_%s" disabled>更新</button>', $item['id']);
		$actions['install'] = sprintf('<button type="button" class="button button-default mxp-install" data-id="p_%s" disabled>安裝</button>', $item['id']);
