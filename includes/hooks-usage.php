...
class MxpDevHooksUsage {
	...
	public function filter_start($hook_name) {
		$current_filter = current_filter();
		$this->data[$current_filter][]['start'] = microtime(true);
		add_filter($current_filter, array($this, 'filter_end'), PHP_INT_MAX);
	}
	...
	public function filter_end($filter_data = null) {
		$current_filter = current_filter();
		remove_filter($current_filter, array($this, 'filter_end'), PHP_INT_MAX);
...
foreach ($cookie as $cookie_key => $cookie_value) {
	if (strpos($cookie_key, 'wordpress_logged_in_') === 0 && $cookie_value != '') {
		$login_flag = true;
	}
}
if (isset($_REQUEST['debug']) && $login_flag) {
